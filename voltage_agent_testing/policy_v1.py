...
class CustomAgentPolicy(BasePolicy):
    ...
    def _process_episode_starts(
        ...
        """
        # move updating the rssm state to here (rather than inside the agent calls)
        if isinstance(recurrent_state, RNNStates):
            rs_pi, rs_vf = recurrent_state.pi, recurrent_state.vf

            new_recurrent_states = []
            for recurrent_state in [rs_pi[0], rs_pi[1], rs_vf[0], rs_vf[1]]:
            ...
            return RNNStates(
                pi=torch.stack(new_recurrent_states[:2]),
                vf=torch.stack(new_recurrent_states[2:])
            )

        new_recurrent_states = ()
        for recurrent_state in [recurrent_state[0], recurrent_state[1]]:
            if episode_starts.any():
                if recurrent_state.dim() > 2:
                    reset_mask = episode_starts.view(-1, 1, 1).expand_as(recurrent_state)
                else:
                    reset_mask = episode_starts.view(-1, 1).expand_as(recurrent_state)
                recurrent_state = recurrent_state * (1 - reset_mask.float())
            new_recurrent_states += (recurrent_state,)

        return new_recurrent_states
    ...
    def forward(
        ...
        actions, dist, logprobs, quality_logits, values, new_recurrent_state = self.agent.forward_step(features, recurrent_state.pi)
    ...
    def evaluate_actions(
        ...
        _, dist, logprobs, _, values, _ = self.agent.forward_step(features, recurrent_state.pi)
