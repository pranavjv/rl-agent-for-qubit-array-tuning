...
class CustomAgentPolicy(BasePolicy):
    ...
    def forward(self, obs: PyTorchObs, recurrent_state: RNNStates, episode_starts: torch.Tensor, deterministic: bool = False) -> Tuple[torch.Tensor]:
...
class CustomRecurrentPPO(RecurrentPPO):
    ...
    def _setup_model(self) -> None:
        ...
        # initialise the actor and critic recurrent states
        self._last_recurrent_states = RNNStates(
            torch.zeros(single_recurrent_shape, device=self.device),
            torch.zeros(single_recurrent_shape, device=self.device),
        )
