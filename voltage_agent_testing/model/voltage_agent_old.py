...
from typing import Optional, Sequence, Type, Dict, Tuple, Union
ModuleType = Optional[Type[nn.Module]]

try:
    from model.base_classes import MLP, CNN
    from model.rssm import RecurrentModel, RecurrentLSTMModel
except ModuleNotFoundError:
    from base_classes import MLP, CNN
    from rssm import RecurrentModel, RecurrentLSTMModel
...
class Agent(nn.Module):
    ...
    def __init__(
        ...
        self.rssm = RecurrentLSTMModel(
            input_dim=feature_dim,
            hidden_dim=recurrent_dim,
            num_layers=num_rssm_layers,
        )
        if print_size:
    ...
    def encode_obs(self, image: torch.Tensor, voltages: torch.Tensor):
