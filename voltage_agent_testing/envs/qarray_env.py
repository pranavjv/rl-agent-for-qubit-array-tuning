...
# simple qarray 2dot env with no voltage obs, use for testing policies
...
class QuantumDeviceEnv(gym.Env):
    ...
    def __init__(self, config_path='qarray_2dot_config.yaml', render_mode=None, **kwargs):
        """
    ...
    def _get_obs(self):
        ...
        if observation.shape != expected_image_shape:
            raise ValueError(f"Image observation shape {observation.shape} does not match expected {expected_image_shape}")
