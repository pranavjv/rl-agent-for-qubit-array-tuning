...
class QuantumDeviceEnv(gym.Env):
    ...
    def __init__(self, config_path='qarray_config.yaml', render_mode=None, **kwargs):
        ...
        print('Random environment created')
    ...
    def reset(self, seed=None, options=None, random_center=True):
        """
        ...
        # Reset random action scaler
        # RANDOMISED
        delta = 0.3
        self.random_scaler = np.random.uniform(1 - delta, 1 + delta, size=self.action_space.shape)
        if self.debug:
    ...
    def step(self, action):
        ...
        # RANDOMISED
        scaled_action = action * self.random_scaler

        self._apply_voltages(scaled_action) #this step will update the voltages stored in self.device_state
    ...
    def _load_model(self):
        ...
        cdd_shape = np.array(Cdd).shape
        cds_shape = np.array(Cds).shape
        ...
        if self.debug:
            ...
            print(f"Cds matrix: {Cds}")
            print(f"Cgd matrix: {Cgd}")
            print(f"Cgs matrix: {Cgs}")
...
if __name__ == "__main__":
    ...
    env.reset(seed=2, random_center=False)
