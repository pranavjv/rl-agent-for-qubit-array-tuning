...
class CapacitanceDataset(data.Dataset):
    ...
    def _build_index(self):
        ...
        for file_idx, (img_file, _) in enumerate(zip(self.image_files, self.cgd_files)):
            # Load images to get batch size and number of channels
            images = np.load(img_file, allow_pickle=True)
            batch_size = images.shape[0]
            num_channels = (
                images.shape[-1] if len(images.shape) == 4 else 1
            )  # (batch, H, W, channels)
    ...
    def _validate_data(self):
        ...
        for idx in check_indices:
            ...
            assert (
                images.ndim == 4
            ), f"Batch {idx}: expected 4D images (batch, H, W, channels), got {images.ndim}D"
            ...
            assert (
                sample_cgd.ndim == 2
            ), f"Batch {idx}: expected 2D CGD matrix, got {sample_cgd.ndim}D"
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.load_to_memory:
            ...
            cgd_matrix = self.memory_cgd_matrices[file_idx][
                local_idx
...
def create_data_loaders(
    ...
    full_dataset = CapacitanceDataset(
        data_dir=data_dir, transform=transform, load_to_memory=load_to_memory
    )
...
if __name__ == "__main__":
    # Test the dataset
    data_dir = (
        "/home/rahul/Summer2025/rl-agent-for-qubit-array-tuning/Swarm/CapacitanceModel/dataset"
    )
    ...
    train_loader, val_loader = create_data_loaders(data_dir, batch_size=4, load_to_memory=False)
