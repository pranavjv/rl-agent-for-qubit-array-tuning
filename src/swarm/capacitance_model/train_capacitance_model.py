...
def calculate_uncertainty_calibration(
    predictions: Tuple[torch.Tensor, torch.Tensor], targets: torch.Tensor
) -> Dict[str, float]:
    ...
    if len(uncertainties_flat) > 1:
        correlation, _ = stats.pearsonr(uncertainties_flat, errors_flat)
        spearman_corr, _ = stats.spearmanr(uncertainties_flat, errors_flat)
    else:
        correlation, spearman_corr = 0.0, 0.0
...
def create_scatter_plots(
    ...
    for i, name in enumerate(target_names):
        ...
        ax1.set_title(
            f"{name} - Predictions vs Actual (Epoch {epoch})",
            fontsize=13,
            fontweight="bold",
        )
        ...
        r2 = 1 - np.sum((targets[:, i] - values[:, i]) ** 2) / np.sum(
            (targets[:, i] - np.mean(targets[:, i])) ** 2
        )
        ...
        if len(bin_uncertainties) > 0:
            ...
            for unc, cov68, count in zip(bin_uncertainties, bin_coverage_68, bin_counts):
                ...
                if n > 0:
                    ...
                    ax2.errorbar(
                        unc,
                        cov68,
                        yerr=[[cov68 - ci_lower], [ci_upper - cov68]],
                        fmt="none",
                        color="steelblue",
                        alpha=0.5,
                        capsize=3,
        ...
        ax2.set_title(
            f"{name} - Uncertainty Calibration (Epoch {epoch})",
            fontsize=13,
            fontweight="bold",
        )
    ...
    if log_wandb and wandb.run is not None:
        try:
            scatter_figures = create_scatter_plots(
                (all_pred_values, all_pred_logvars), all_targ, epoch
            )
...
def main():
    ...
    parser.add_argument(
        "--mse_weight", type=float, default=1.0, help="Weight for MSE loss component"
    )
    parser.add_argument(
        "--nll_weight",
        type=float,
    ...
    parser.add_argument(
        "--wandb_project",
        type=str,
        default="capacitance-prediction",
    ...
    loss_fn = create_loss_function(mse_weight=args.mse_weight, nll_weight=args.nll_weight)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_metrics = validate_epoch(
            model, val_loader, loss_fn, device, epoch, log_wandb=not args.no_wandb
        )
        ...
        print(
            f"Val Targets - Cgd_1_2: {val_metrics['Cgd_1_2_mae']:.4f}, "
        ...
        if epoch % args.save_freq == 0 or is_best:
            save_checkpoint(model, optimizer, epoch, best_val_loss, output_dir, is_best)
