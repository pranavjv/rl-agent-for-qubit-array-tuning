...
class CapacitancePredictor:
    ...
    def bayesian_update(self, i: int, j: int, ml_estimate: float, ml_variance: float):
        ...
        if new_var > current_var:
            warnings.warn(
                f"Posterior variance increased for element ({i},{j}). "
    ...
    def _find_neighbor(self, dot_idx: int, exclude: List[int] = None) -> Union[int, None]:
        ...
        for neighbor in candidates:
            if 0 <= neighbor < self.n_dots and neighbor not in exclude and neighbor != dot_idx:
                return neighbor
    ...
    def get_full_matrix(
        self, return_variance: bool = False
    ...
    def get_confidence_interval(
        self, i: int, j: int, confidence_level: float = 0.95
    ) -> Tuple[float, float]:
        """
...
if __name__ == "__main__":
    ...
    for i in range(n_dots):
        for j in range(n_dots):
            if i == j:
                ...
                prior_dict[(i, j)] = (0.0, 0.1)  # Distant pairs
    ...
    predictor.update_from_scan(
        (1, 2),
        [
            (ml_estimates[0], log_variances[0]),
            (ml_estimates[1], log_variances[1]),
            (ml_estimates[2], log_variances[2]),
        ],
    )
