...
def save_metadata(
    config: GenerationConfig, output_dir: Path, generation_stats: Dict[str, Any]
) -> None:
...
def generate_dataset(config: GenerationConfig) -> None:
    ...
    logger.info(
        f"Starting dataset generation: {config.total_samples} samples with {config.workers} workers"
    )
    ...
    with ProcessPoolExecutor(max_workers=config.workers) as executor:
        # Submit all jobs
        future_to_id = {
            executor.submit(generate_sample, i, config): i for i in range(config.total_samples)
        }
        ...
        for future in as_completed(future_to_id):
            ...
            try:
                ...
                if (
                    len(batch_samples) >= config.batch_size
                    or successful_samples + failed_samples == config.total_samples
                ):

                    if save_batch(current_batch_id, batch_samples, output_dir):
                        saved_batches += 1
                        logger.info(
                            f"Saved batch {current_batch_id} ({len(batch_samples)} samples)"
                        )
...
def main():
    ...
    parser.add_argument(
        "--batch_size", type=int, default=1000, help="Number of samples per batch file"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
